...
public class KeyMouseExam extends JApplet implements MouseListener, MouseMotionListener, KeyListener {
   private int x,y;//이미지를 출력할 x,y좌표값
    ...
    private boolean flag=false; // 이미지가 처음 실행할 땐 안보이게 false로씀
    ...
    public void init() {
        ...
        this.addKeyListener(this);//현재 클래스에 키 이벤트 기능 추가
    ...
    // 컴퍼넌트창에서 마우스 버튼을 클릭(눌렀다 놓을 때)했을 때 호출
    @Override
    public void mouseClicked(MouseEvent e) {
    flag = true; //클릭할 때 그림 true로 설정해서 보여지게함.
        x = e.getX();
        y = e.getY();
